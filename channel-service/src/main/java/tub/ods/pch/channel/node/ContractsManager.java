...
public class ContractsManager {
	...
	public ContractsManager(EthRpcProperties rpcProperties, Web3j web3j, Credentials credentials,
		...
		try {
			// Address channelManagerToken = (Address) channelManager.token2();
			RemoteFunctionCall<String> addr = channelManager.token();
			Address channelManagerToken = channelManager.token2();
			Preconditions.checkState(new Address(schToken.getContractAddress()).equals(channelManagerToken),
					"Wrong token contract %s != %s", schToken.getContractAddress(), channelManagerToken);
		} catch (Exception e) {
	...
	public <C extends Contract> C load(Class<C> contractClass, Address address) {
		try {
			...
			Method method = contractClass.getDeclaredMethod("load", String.class, Web3j.class, TransactionManager.class,
					BigInteger.class, BigInteger.class);
			Object contract = method.invoke(null, address.toString(), web3j, transactionManager,
					rpcProperties.getGasPrice(), rpcProperties.getGasLimit());
...
//            }
	...
	public <C extends Contract> DeployingContract<C> startDeployment(Class<C> contractClass, Type... args) {
		...
		try {
			String transactionHash = transactionManager.sendTransaction(rpcProperties.getGasPrice(),
					rpcProperties.getGasLimit(), null, binary + encodedConstructor, BigInteger.ZERO)
					.getTransactionHash();
	...
	public <C extends Contract> Optional<C> checkIfDeployed(DeployingContract<C> deployingContract) {
		try {
			TransactionReceipt receipt = checkError(
					web3j.ethGetTransactionReceipt(deployingContract.transactionHash).send());
	...
	private <T extends Contract> T create(Class<T> type, TransactionReceipt rc) {
		try {
			...
			T contract = constructor.newInstance(null, web3j, transactionManager, rpcProperties.getGasPrice(),
					rpcProperties.getGasLimit());
...
//	Added new casting
	...
	public ChannelApiStub load(Class<ChannelApiStub> contractClass, CompletableFuture<String> channelApi) {
		try {
			// String contractAddress, Web3j web3j, TransactionManager transactionManager,
			// BigInteger gasPrice, BigInteger gasLimit
			Method method = contractClass.getDeclaredMethod("load", String.class, Web3j.class, TransactionManager.class,
