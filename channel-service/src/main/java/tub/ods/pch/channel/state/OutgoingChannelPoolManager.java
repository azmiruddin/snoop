...
import tub.ods.pch.channel.state.OutgoingChannelBean;
import tub.ods.pch.channel.state.OutgoingChannelPoolBean;
...
public class OutgoingChannelPoolManager {
	...
	private void cycle() {
		while (!Thread.interrupted()) {
			try {
				...
				registry.all().forEach(channel -> indexed
						.computeIfAbsent(channel.getChannel().getSenderAddress(), sender -> new HashMap<>())
						.computeIfAbsent(channel.getChannel().getReceiverAddress(), receiver -> new ArrayList<>())
						.add(channel));
				indexed.forEach(
						(sender, map) -> map.forEach((receiver, channels) -> managePool(sender, receiver, channels)));
	...
	private void managePool(Address senderAddress, Address receiverAddress, List<OutgoingChannelState> channels) {
		...
		if (channelProperties == null) {
			channels.stream().filter(ch -> ch.isActive() && ch.getOpenBlock() > 0)
			...
			if (notClosedOrClosing < channelProperties.getMinActiveChannels()) {
				...
				OutgoingChannelState newChannel = new OutgoingChannelState(senderAddress, clientAddress,
						receiverAddress, channelProperties.getBlockchainProperties());
	...
	public void start() throws Exception {
		Retriable.wrapTask(() -> {
			...
			loadChannels();
				...
				.withErrorMessage("Failed to load state from persistent store").call();
	...
	private void loadPools() {
		for (OutgoingChannelPoolBean bean : poolRepository.all()) {
			if (!ethereumConfig.hasAddress(bean.getSender())) {
				...
				createOrUpdatePool(bean.getSender(), bean.getReceiver(), new ChannelPoolProperties(bean));
			}
		}
	}
	...
	private void loadChannels() {
		for (OutgoingChannelBean bean : channelRepository.all()) {
			if (bean.getStatus() == OutgoingChannelState.Status.SETTLED)
				continue;
			...
			if (!state.isPresent()) {
				...
				registry.register(channel,
						getPolicy(channel.getChannel().getSenderAddress(), channel.getChannel().getReceiverAddress()));
			} else {
				channel = state.get();
			}
