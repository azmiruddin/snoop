...
public class RedisBufferLayer extends DistributedBufferLayerSecure<long[]> {
    ...
    protected boolean updateSecure(DistributedBlockEntry<long[], ?> prBlockEntry,byte[] valueRedis) {
        ...
        jedis.zadd(index, 0, key + value);
        jedis.set(value.getBytes(),valueRedis); //key // value
	...
	@Override
	public void updateRemote(DistributedBlockEntry<long[], ?> prBlockEntry) {
		 long[] record = prBlockEntry.getBlockRecord();
	        int[] recordKey = makeKey(record);
	...
	public void updateRemote(DistributedBlockEntry<long[], ?> prBlockEntry,byte[] valueRedis) {
		 long[] record = prBlockEntry.getBlockRecord();
	        int[] recordKey = makeKey(record);
